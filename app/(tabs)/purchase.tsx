...
export default function PurchaseScreen() {
  ...
  async function addToCart(product: Product) {
    const lastPrice = await purchaseService.lastPriceFor(product.id);
    nextKey.current += 1;
    setCart((prev) => [
      ...prev,
  ...
  function addQuickQuantity(key: string, amount: number) {
    setCart((prev) => prev.map((l) => (l.key === key ? { ...l, quantity: l.quantity + amount } : l)));
  }
  ...
  function confirmSave() {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: 'Save purchase', onPress: performSave },
      ]
    );
  }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.xl, gap: spacing.md }}>
        {cart.length === 0 ? (
          ...
          >
            <Ionicons name="cube-outline" size={22} color={colors.inkFaint} />
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={{ flexDirection: 'row', gap: 6, marginTop: 8 }}>
                    {QUICK_ADD.map((amount) => (
                      <Pressable
                        key={amount}
                        onPress={() => addQuickQuantity(line.key, amount)}
                        style={[styles.qtyChip, { borderColor: colors.lineStrong, borderRadius: radius.control }]}
                      >
                        <Text style={{ color: colors.inkSoft, fontSize: 11, fontWeight: '700' }}>+{amount}</Text>
                      </Pressable>
                    ))}
                  </View>
                </View>
        ...
        <Pressable
          ...
          style={[styles.addButton, { borderColor: colors.purchase, borderRadius: radius.card }]}
        >
          <Ionicons name="add" size={18} color={colors.purchase} />
          <Text style={{ color: colors.purchase, fontWeight: '700', fontSize: 14 }}>
      ...
      <View style={[styles.footer, { borderTopColor: colors.line, backgroundColor: colors.surface }]}>
        ...
        <Pressable
          ...
          style={[
            styles.saveButton,
            { backgroundColor: colors.purchase, borderRadius: radius.control, opacity: cart.length === 0 || saving ? 0.6 : 1 },
          ]}
        >
          <Text style={{ color: colors.onPurchase, fontSize: 15, fontWeight: '700' }}>
...
const styles = StyleSheet.create({
  ...
  qtyChip: { borderWidth: StyleSheet.hairlineWidth, paddingHorizontal: 9, paddingVertical: 5 },
  priceRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
